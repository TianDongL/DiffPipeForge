...
export function ModelConfig({ data, onChange }: ModelConfigProps) {
    ...
    const getDefaultsForModel = (type: string) => {
        ...
        switch (type) {
            ...
            case 'minimax_h3':
                return { ...base, diffusion_model_dtype: '', timestep_sample_method: 'uniform', shift: 8, image_shift: 1, cfg: 4, merge_adapters: '' };
            default:
                return base;
        }
    };
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        ...
        if (modelType === 'minimax_h3') {
            if (e.target.name === 'cfg' && value !== '' && Number(value) > 1) {
                updates.merge_adapters = '';
            } else if (e.target.name === 'merge_adapters' && typeof value === 'string' && value.trim() !== '') {
                updates.cfg = '';
            }
        }
    ...
    const handlePickPath = async (name: string, isFolder: boolean = false) => {
        ...
        try {
            ...
            if (!result.canceled && result.filePaths.length > 0) {
                const updates: Record<string, any> = { [name]: result.filePaths[0] };
                if (modelType === 'minimax_h3' && name === 'merge_adapters') {
                    updates.cfg = '';
                }
                onChange({ ...data, ...updates });
            }
    ...
    const renderFields = () => {
        switch (modelType) {
            ...
            case 'ideogram4':
                return (
                    <>
                        <PathInput label={t('model.diffusion_model')} helpText={t('help.diffusion_model')} name="diffusion_model" data={data} handleChange={handleChange} handlePickPath={handlePickPath} openTitle={t('project.open')} placeholder="ideogram4_fp8_scaled.safetensors" />
                        <PathInput label={t('model.vae_path')} helpText={t('help.vae_path')} name="vae" data={data} handleChange={handleChange} handlePickPath={handlePickPath} openTitle={t('project.open')} placeholder="flux2-vae.safetensors" />
            ...
            case 'anima':
                return (
                    <>
                        <PathInput label={t('model.transformer_path')} helpText={t('help.transformer_path')} name="transformer_path" data={data} handleChange={handleChange} handlePickPath={handlePickPath} openTitle={t('project.open')} placeholder="anima-preview.safetensors" />
                        <PathInput label={t('model.vae_path')} helpText={t('help.vae_path')} name="vae_path" data={data} handleChange={handleChange} handlePickPath={handlePickPath} openTitle={t('project.open')} placeholder="qwen_image_vae.safetensors" />
            ...
            case 'minimax_h3':
                return (
                    <>
                        ...
                        <GlassInput label={t('model.minimax_cfg')} helpText={t('help.minimax_cfg')} name="cfg" type="number" min="1" step="0.1" value={formatNum(data.cfg ?? '')} onChange={handleChange} placeholder="4" />
                        ...
                        <div className="col-span-2 rounded-lg border border-blue-400/20 bg-blue-400/10 p-3 text-sm text-blue-700 dark:text-blue-200">
                            {t('help.minimax_distillation')}
                        </div>
