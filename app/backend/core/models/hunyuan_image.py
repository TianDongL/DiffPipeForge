...
import sys
import os
sys.path.append(os.path.join(os.path.abspath(os.path.dirname(__file__)), '../submodules/HunyuanImage-2.1'))
...
class HunyuanImagePipeline(BasePipeline):
    ...
    def __init__(self, config):
        ...
        config = HunyuanVAE2D.load_config(str(configs_dir / 'hunyuan_image' / 'vae_config.json'))
        ...
        self.tokenizer = transformers.Qwen2Tokenizer.from_pretrained(str(configs_dir / 'qwen_image' / 'tokenizer'), local_files_only=True)

        text_encoder_config = transformers.Qwen2_5_VLConfig.from_pretrained(str(configs_dir / 'qwen_image' / 'text_encoder'), local_files_only=True)
        with init_empty_weights():
        ...
        self.byt5_tokenizer = transformers.ByT5Tokenizer.from_pretrained(str(configs_dir / 'hunyuan_image' / 'byt5-small'), local_files_only=True)

        byt5_config = transformers.T5Config.from_pretrained(str(configs_dir / 'hunyuan_image' / 'byt5-small'), local_files_only=True)
        with init_empty_weights():
