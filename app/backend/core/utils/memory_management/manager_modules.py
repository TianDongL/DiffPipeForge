...
Performance-optimized version v4: Triple Buffering + Async Grad Transfer
  - Triple-buffered weight slots with clock-based round-robin
  - Forward: transfer_stream overlaps next layer's H2D with current compute
  - Backward: transfer_grad_stream moves grads to CPU asynchronously
  - 6 CUDA events for precise inter-stream synchronization
"""
    ...
    if device not in _DEVICE_STATE:
        with torch.cuda.device(device):
            _DEVICE_STATE[device] = {
                ...
                # ping-pong buffers (Triple Buffering)
                "w_buffers": [None, None, None],
                "b_buffers": [None, None, None],
                "w_bwd_buffers": [None, None, None],
                # device-side staging for grads to be sent to CPU
                "w_grad_buffers": [None, None, None],
                "b_grad_buffers": [None, None, None],
                # clocks
                "forward_clk": 0,
                "backward_clk": 0,
            }
...
def _is_ao_quantized_tensor(t: Optional[torch.Tensor]) -> bool:
    ...
    try:
        ...
        pass
    for attr in (
        "_scale", "_scales", "_zero_point", "_zp",
...
class _BouncingLinearFn(torch.autograd.Function):
    ...
    def forward(ctx, x, weight_cpu, bias_cpu, device: torch.device):
        ...
        with torch.cuda.stream(ts):
            ...
            state["forward_clk"] = (state["forward_clk"] + 1) % 3
            ev_tx_f.record()
    ...
    def backward(ctx, grad_out):
        ...
        with torch.cuda.stream(transfer_stream):
            ...
            state["backward_clk"] = (state["backward_clk"] + 1) % 3
            ev_tx_b.record()
...
class _BouncingConv2dFn(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx, x, weight_cpu, bias_cpu, device: torch.device,
        ...
        if device.type != "cuda":
            ...
            out = F.conv2d(
                x.to("cpu"),
                _materialize_conv_weight(weight_cpu, cpu_dev),
                bias_cpu, stride, padding, dilation, groups,
            )
            ctx.save_for_backward(x.to("cpu"), weight_cpu, bias_cpu)
            ctx.meta = (cpu_dev, stride, padding, dilation, groups, target_dtype)
            return out.to(x.device)
    ...
    def backward(ctx, grad_out):
        ...
        grad_input = conv2d_input(
            x.shape, w_bwd_buffers[idx], grad_out.to(dtype=target_dtype),
            stride=stride, padding=padding, dilation=dilation, groups=groups,
        )
...
class ConvLayerMemoryManager(BaseLayerMemoryManager):
    def __init__(self, module: nn.Module, manager: "MemoryManager"):
        ...
        # 2) Capture conv params at attach time
        stride = (
            self.module.stride
