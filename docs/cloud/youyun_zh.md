...
6. 在应用或自定义开放端口设置中开放 `7860`。已有的 JupyterLab `8888` 可以保留，不要用它覆盖 7860。
7. 创建并启动实例。
...
1. 在实例卡片的应用/自定义服务区域打开 7860 对应的 WebUI。
2. 首次启动等待页面完整加载，不要因为模型列表为空而重装环境；模型文件位于平台的只读模型盘，不会复制到仓库目录。
3. 确认左侧能看到“数据集设置”“训练配置”“开始训练”“全局监控”和“系统诊断”。
4. 打开“系统诊断”，确认 Python 环境已就绪。
